...
<Tabs groupId="ssh-auth" items={['Private Key (Recommended)', 'Password', 'ssh-agent']}>
  ...
  <Tab value="ssh-agent">
    Enable **Use ssh-agent** to authenticate with identities from your local agent.
    Leave **Agent Socket Path** empty to use `SSH_AUTH_SOCK`, or set a custom Unix socket path when DBX should use a specific agent.
  </Tab>
</Tabs>
